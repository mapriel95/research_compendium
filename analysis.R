...
q <- c("packageid=knb-lter-pal.219.5&entityid=002f3893385f710df69eeebe893144ff","packageid=knb-lter-pal.220.7&entityid=e03b43c924f226486f2f0ab6709d2381", "packageid=knb-lter-pal.221.8&entityid=fe853aa8f7a59aa84cdd3197619ef462")

files <- c("adelie.csv", "gentoo.csv", "chinstrap.csv")

download.file( paste0(url, q), files, "libcurl" )

raw.data <- do.call(rbind,
                    lapply(files,
                           read.csv,
                           na.strings = c("NA", "", ".")))
...
write.csv(cleanedData,
          "~/data.csv",
          row.names = F)

sum_table <- cleanedData |>
